<?php
namespace Common\Controllers\Uploader;
...
class PluploadHandler
{
    ...
    {
        parse_str( ((isset($_POST['data']))? $_POST['data'] : ""), $data );
    ...
    protected function geTname($file_name){

        $extension = pathinfo($file_name, PATHINFO_EXTENSION);
        $basenames = bin2hex(random_bytes(36));
        $file_name = sprintf('%s.%0.8s', $basenames, $extension);
    ...
    protected function handleFile($file_name)
    {
        ...
        $file_path = $this->getTargetPathFor($file_name);
        $tmp_paths = $this->writeUploadTo($file_path . ".part");
        $file_path = $this->getTargetPathFor($this->geTname($file_name));

        return $this->rename($tmp_paths, $file_path, $file_name);
    }


    protected function rename($tmp_path, $file_path, $file_name)
    {
